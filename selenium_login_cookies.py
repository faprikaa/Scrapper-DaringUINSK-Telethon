...
prefs={"download.default_directory":f"{down_path}", "download.directory_upgrade": True};
options.add_experimental_option("prefs",prefs);
if OS_type == "Linux":
    options.add_argument("--headless=new")
    options.add_argument("--no-sandbox")
ser = Service(f"{driver_path}")
...
def take_ss(browser, full_id):
    ...
    height = y2 + h
    im = Image.open(names)
    im = im.crop((50, int(1), int(579), int(h)))
    im.save(name)
    return(name)
    ...
    for full_id in all_id:
        ...
        try:
            if jenis=="Tugas":
                ...
                await bot.send_message(chat, f"Jenis postingan baru : {jenis}")
        except KeyError:
            if jenis=="Tugas":
                pic_name = take_ss(browser, full_id)
                loop.run_until_complete(tugasbot(full_id, pic_name))
...
def scrape(ps):
    ...
    with open(jsonfile, "r") as file:
        data = json.load(file)
    global data_new
    data_new = json_returner(data)

    send_var(browser, soup, bot, chat)
